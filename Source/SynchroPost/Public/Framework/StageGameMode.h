...
private:
	...
	void AssembleCommon(const UStageDataAsset* StageData);
	// 전투 스테이지에만 해당하는 구성 요소를 배치
	void AssembleCombat(const FStageNode& Node, const UCombatStageDataAsset* CombatStageData);

	void SpawnProps(const UStageDataAsset* StageData);
	void SpawnEntities(const UStageDataAsset* StageData);
	void SpawnExits(const UStageDataAsset* StageData);
